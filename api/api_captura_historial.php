...
// 2. OBTENER ID DEL USUARIO ACTIVO
// Nota: Ajusta 'usuario_id' por el nombre real de tu variable de sesión si es diferente (ej. 'id' o 'id_usuario')
// 2. OBTENER ID DEL USUARIO ACTIVO (Atrapa-todo)
$usuario_id = $_SESSION['id_usuario'] ?? $_SESSION['usuario_id'] ?? $_SESSION['id'] ?? $_SESSION['user_id'] ?? 0;
// 3. FILTRAR POR "HOY" Y POR "USUARIO"
$hoy = date('Y-m-d'); 

$sql = "SELECT * FROM historial_rapido 
        WHERE estatus = 1 
        AND DATE(fecha) = ? 
        AND usuario_id = ?  /* <--- NUEVO: Filtro para que no choquen las capturas */
        ORDER BY id DESC";

try {
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$hoy, $usuario_id]); // <--- NUEVO: Pasamos el ID del usuario a la consulta
    ...
    if ($stmt->rowCount() > 0) {
        ...
        echo "<tr><td colspan='6' class='text-center py-12 text-slate-300 italic text-xs'>
            <div class='flex flex-col items-center gap-3'>
                ...
                <span>Aún no has escaneado nada hoy</span>
            </div>
        </td></tr>";
    }
} catch (Exception $e) {
